  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 to-indigo-50">
      ...
      <header className="border-b border-indigo-100 bg-white/80 px-6 py-5 backdrop-blur">
        <div className="mx-auto max-w-5xl flex items-center justify-between gap-4">
          <div className="flex items-center gap-3">
            ...
            <span className="hidden text-sm text-slate-500 sm:inline">Download Skills</span>
          </div>
          <button
            type="button"
            onClick={onBack}
      ...
      <main className="mx-auto max-w-5xl px-6 py-8">
        <div className="mb-6">
          <h1 className="mb-1 text-2xl font-bold text-slate-900">Download Skill Packs</h1>
          <p className="text-slate-500">
            Select the skills you need. Each skill downloads as a{" "}
            <code className="rounded bg-slate-100 px-1 text-xs">SKILL.md</code> file — drop it into
            your project&apos;s <code className="rounded bg-slate-100 px-1 text-xs">skills/</code>{" "}
            folder and reference it from your agent&apos;s{" "}
            <code className="rounded bg-slate-100 px-1 text-xs">AGENTS.md</code>.
          </p>
        </div>
        ...
        {grouped.length === 0 && (
          <p className="py-12 text-center text-sm text-slate-400">No skills match your search.</p>
        )}
        {grouped.map((cat) => (
        ...
        {/* Sticky download bar */}
        <div className="sticky bottom-0 -mx-6 border-t border-gray-200 bg-white/90 px-6 py-4 backdrop-blur">
            ...
            <div>
              ...
              {!downloadError && selectedIds.size > 0 && (
                <p className="text-sm text-slate-500">
                  ...
                  <code className="rounded bg-slate-100 px-1 text-xs">skills.zip</code>
                </p>
              )}
            </div>
            <button
